...
;
(function ($) {
  //wrapping first word of school term name in a span in footer info
  $('.view-footer-info .views-field-name .field-content').each(function(){
       var me = $(this);
       me.html(me.html().replace(/^(\w+)/, '<span>$1</span>'));
  });

  // search icon
  $('.school-sub-menu .mobile-search').click(function(e) {
    e.preventDefault();
    $(this).toggleClass('open');
    $('#block-headersearch-2').toggleClass('open');
  });

  // Archived Minutes
  $(".meeting-minutes .archived-minutes").click(function(e) {
    e.preventDefault();
    if($(this).hasClass('collapsed')) {
      $(this).removeClass('collapsed');
      $(this).addClass('expanded');
      $(this).siblings('.archive').show(200);
      $(this).text('Close Archived Meetings');
    }
    else {
      $(this).addClass('collapsed');
      $(this).removeClass('expanded');
      $(this).siblings('.archive').hide(200);
      $(this).text('View Archived Meetings');
    }
  });

  $(document).ready(function(){
    $('div.fc-day-grid.fc-unselectable').off();
    $('div.fc-day-grid').off();
  });
  ...
  $(window).scroll(function(){
    if($(document).scrollTop() > 5) {
      ...
      $('#block-headersearch-2').removeClass('open');
      $('.school-sub-menu .mobile-search').removeClass('open');
    } else {
      $('header').removeClass('fixed');
    }
  });

  // Main container padding for fixed menu
  $(window).ready(function() {
    if (window.location.search.length) {
      $('html,body').animate({
         scrollTop: $('.view-building-files .views-field-field-files').offset().top - 10
      });
    }
    var height = $('.header-wrapper').height();
    $('.main-container').css('padding-top', height);
    if(window.innerWidth < 767 && $(".info-banner-mobile")[0]) {
      var notificationheight = $('.info-banner-mobile').height();
      $('.main-container').css('padding-top', height+notificationheight);
    }
    if(window.innerWidth >= 767) {
      $('#block-testing-main-menu ul.menu').show();
      $('#block-testing-main-menu h2').removeClass('open');
      $('.school-sub-menu ul.menu').show();
      $('.school-sub-menu h2').removeClass('open');
    }
    if(window.innerWidth < 767) {
      // mobile main menu expand/collapse
      $('#block-testing-main-menu h2').click(function(e) {
        $(this).toggleClass('open');
        $(this).siblings('ul.menu').slideToggle(200);
      });
      $('.school-sub-menu h2').click(function(e) {
        $(this).toggleClass('open');
        $(this).siblings('ul.menu').slideToggle(200);
      });
      $('#block-testing-main-menu ul.menu').hide();
      $('.school-sub-menu ul.menu').hide();

      // open search close menu / open menu close search
      ...
      $('.navbar-toggle').click(function(e) {
        ...
        $('#search-collapse').attr('aria-expanded','false');
...
/*
(function($) {
  Drupal.behaviors.increment = {
    attach: function (context) {
      $("input[type=time]").on("change", function() {
        var arr_one = [0, 15, 30, 45];
        if (arr_one.indexOf(this.valueAsDate.getMinutes()) == -1) {
          var minutes = Math.round(this.valueAsDate.getMinutes() * 10 / 15) * 15;
          if (minutes >= 60) {
            minutes = 0;
          }
          var time = this.valueAsDate.setMinutes(minutes);
